...
const prepareUploadFile = async (file, config, basePath, GCS) => {
  ...
  if (!config.uniform) {
    ...
    //custom code - if file name contains public make it public else make it private
    const isPublicFile = fullFileName.includes('public');
    fileAttributes.public = isPublicFile;
  }
